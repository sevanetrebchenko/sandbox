...
float G(vec4 p) {
    ...
    if (shadowCoordinate.w > 0.0f) {
        ...
        if (InRange(shadowCoordinate.x, 0.0f, 1.0f) && InRange(shadowCoordinate.y, 0.0f, 1.0f)) {
            ...
            float shadowBias = max(shadowBiasMax * (1.0f - dot(normalize(worldNormal.xyz), normalize(-lightDirection))), shadowBiasMin);
...
void main(void) {
    ...
    // Ambient.
    vec3 Ka = ambientCoefficient;
    vec3 ambientComponent = Ka;

    // Diffuse.
    vec3 Kd = diffuseCoefficient;
