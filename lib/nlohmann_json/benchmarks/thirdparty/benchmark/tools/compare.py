...
def check_inputs(in1, in2, flags):
    ...
    if in1_kind == IT_JSON and in2_kind == IT_JSON and len(flags) > 0:
        print("WARNING: passing optional flags has no effect since both "
              "inputs are JSON")
    if output_type is not None and output_type != 'nlohmann_json':
...
class TestParser(unittest.TestCase):
    def setUp(self):
        ...
        testInputs = os.path.join(
            ...
            'gbench',
            'Inputs')
        self.testInput0 = os.path.join(testInputs, 'test1_run1.nlohmann_json')
        self.testInput1 = os.path.join(testInputs, 'test1_run2.nlohmann_json')
